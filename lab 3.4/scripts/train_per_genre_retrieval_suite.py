...
def main() -> None:
    ap = argparse.ArgumentParser(description="Train per-genre scratch retrieval-fusion models and aggregate their production pack.")
    ap.add_argument("--out-root", type=Path, default=Path.home() / "Desktop" / "dggr_per_genre_retrieval_suite")
    ...
    script = REPO_ROOT / "lab 3.4" / "scripts" / "train_scratch_retrieval_fusion.py"
    ...
    report_lines = [
        ...
        "",
        "- Each target genre was trained as its own scratch retrieval-conditioned accompaniment generator.",
