...
class TrainConfig:
    ...
    out_root: Path = REPO_ROOT / "Desktop Outputs" / "dggr_retrieval_body_style_runs"
    epochs: int = 2
    batch_size: int = 4
...
def train(cfg: TrainConfig) -> Dict[str, Any]:
    ...
    out_dir = Path(cfg.out_root) / f"retrieval_body_style_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    out_dir.mkdir(parents=True, exist_ok=True)
    ...
    report_lines = [
        "# Scratch Retrieval Body-Style Diagnosis",
        "",
        ...
        "- New family: retrieval-conditioned accompaniment generator from scratch using source structure, donor texture, and explicit body/groove losses.",
