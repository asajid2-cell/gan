...
class RetrievalFusionDataset(Dataset):
    ...
    def __getitem__(self, item: int) -> Dict[str, torch.Tensor]:
        ...
        if prev_idx is None:
            ...
            prev_mel = np.asarray(self.arrays["mel"][int(prev_idx)], dtype=np.float32)[:, : self.max_frames]
        ...
        return {
            ...
            "has_prev": torch.tensor(0.0 if prev_idx is None else 1.0, dtype=torch.float32),
        }
...
class RetrievalFusionUNet(nn.Module):
    def __init__(self, in_ch: int, num_genres: int, base_ch: int = 48):
        ...
        self.film = nn.Linear(base_ch * 4 + 64 + 1, base_ch * 8)
    ...
    def forward(
        ...
        x = torch.cat([struct_mel, donor_mel, prev_mel, cond_feat], dim=1)
        e1 = self.enc1(x)
        e2 = self.enc2(self.down1(e1))
        h = self.mid(self.down2(e2))
        donor_vec = self.donor_head(donor_mel).flatten(1)
        prev_vec = self.prev_head(prev_mel).flatten(1)
        film = self.film(torch.cat([donor_vec, prev_vec, self.genre_emb(genre_idx), has_prev[:, None]], dim=-1))
    ...
    *,
    ...
    for i, chunk in enumerate(chunks):
        ...
        pred = model(
            ...
            torch.tensor([float(has_prev)], dtype=torch.float32, device=device),
        )
        ...
        if i > 0:
            ...
            blend_cols = min(24, pred_mel.shape[1], prev_norm.shape[1])
            pred_mel[:, :blend_cols] = 0.65 * prev_norm[:, -blend_cols:] + 0.35 * pred_mel[:, :blend_cols]
    ...
    full_mel = _assemble_mel_context_trim(out_mels, trim_frames)
...
class TrainConfig:
    ...
    out_root: Path = REPO_ROOT / "Desktop Outputs" / "dggr_retrieval_body_style_cont_runs"
    epochs: int = 4
    batch_size: int = 4
...
def train(cfg: TrainConfig) -> Dict[str, Any]:
    ...
    out_dir = Path(cfg.out_root) / f"retrieval_body_style_cont_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    out_dir.mkdir(parents=True, exist_ok=True)
    ...
    model = RetrievalFusionUNet(in_ch=17, num_genres=len(genre_to_idx), base_ch=int(cfg.base_ch)).to(device)
    ...
    for epoch in range(1, int(cfg.epochs) + 1):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            pred = model(struct, donor, prev_mel, cond_feat, genre, has_prev)
            ...
            loss_cont = (
                has_prev.mean() * F.l1_loss(pred_lead, prev_tail) +
    ...
    report_lines = [
        "# Scratch Retrieval Body-Style Continuation Diagnosis",
        "",
        ...
        "- New family: retrieval-conditioned accompaniment generator from scratch using source structure, donor texture, explicit body/groove losses, and previous-chunk continuation conditioning.",
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Train a scratch retrieval-conditioned accompaniment generator with continuation conditioning and benchmark it.")
    ap.add_argument("--out-root", type=Path, default=REPO_ROOT / "Desktop Outputs" / "dggr_retrieval_body_style_cont_runs")
