...
def main() -> None:
    ap = argparse.ArgumentParser(description="Train per-genre scratch retrieval body-style continuation models and aggregate their production pack.")
    ap.add_argument("--out-root", type=Path, default=REPO_ROOT / "Desktop Outputs" / "dggr_per_genre_retrieval_body_style_cont_suite")
    ...
    script = REPO_ROOT / "lab 3.4" / "scripts" / "train_scratch_retrieval_body_style_continuation.py"
    ...
    report_lines = [
        "# Per-Genre Retrieval Body-Style Continuation Suite",
        "",
        "- Each target genre was trained as its own scratch retrieval-conditioned accompaniment generator with explicit body and groove retention losses plus previous-chunk continuation conditioning.",
