...
class RetrievalFusionUNet(nn.Module):
    ...
    def forward(self, struct_mel: torch.Tensor, donor_mel: torch.Tensor, cond_feat: torch.Tensor, genre_idx: torch.Tensor) -> torch.Tensor:
...
def generate_longform(
    ...
    *,
    ...
    for i, chunk in enumerate(chunks):
        mel = extract_bigvgan_mel_np(chunk["audio"], sr=DIFFUSION_SR)
        ...
        pred_t = torch.from_numpy(pred_mel[None, None, :, :]).to(device)
        audio = np.asarray(vocode_bigvgan(pred_t, float(mel_min), float(mel_max), vocoder, device), dtype=np.float32).reshape(-1)
        out_wavs.append(audio)
    return assemble_audio_crossfade(out_wavs, overlap_seconds=float(overlap_seconds), sr=DIFFUSION_SR)
...
def benchmark_checkpoint(
    *,
    ...
    for row in all_rows:
        ...
        row["overall"] = float(
            0.45 * row["target_margin"] +
            0.20 * row["target_conf"] +
            0.20 * row["fullness"] +
            0.15 * row["structure"] +
            0.25 * row["separation"] -
            0.25 * row["warble"]
...
class TrainConfig:
    ...
    out_root: Path = Path.home() / "Desktop" / "dggr_retrieval_fusion_runs"
    epochs: int = 2
    batch_size: int = 4
...
def train(cfg: TrainConfig) -> Dict[str, Any]:
    ...
    out_dir = Path(cfg.out_root) / f"retrieval_fusion_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    out_dir.mkdir(parents=True, exist_ok=True)
    ...
    for epoch in range(1, int(cfg.epochs) + 1):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            loss = loss_l1 + 0.25 * loss_dt + 0.20 * loss_df + 0.30 * loss_judge + 0.20 * loss_timbre
            opt.zero_grad(set_to_none=True)
            loss.backward()
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(val_loader):
                if batch_idx >= int(cfg.val_batches):
                    break
                pred = model(batch["struct_mel"].to(device), batch["donor_mel"].to(device), batch["cond_feat"].to(device), batch["genre_idx"].to(device))
    ...
    report_lines = [
        "# Scratch Retrieval Fusion Diagnosis",
        "",
        ...
        "- Failed scratch family: pure structure diffusion from scratch collapsed across genres under a unified judge.",
        "- New family: retrieval-conditioned accompaniment generator from scratch using source structure and target-genre donor texture.",
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Train a scratch retrieval-conditioned accompaniment generator and benchmark it.")
    ap.add_argument("--out-root", type=Path, default=Path.home() / "Desktop" / "dggr_retrieval_fusion_runs")
