...
class RetrievalFusionDataset(Dataset):
    ...
    def __getitem__(self, item: int) -> Dict[str, torch.Tensor]:
        ...
        if prev2_idx is None:
            ...
            prev2_mel = np.asarray(self.arrays["mel"][int(prev2_idx)], dtype=np.float32)[:, : self.max_frames]
...
def train(cfg: TrainConfig) -> Dict[str, Any]:
    ...
    report_lines = [
        ...
        "- New family: retrieval-conditioned accompaniment generator from scratch using source structure, donor texture, explicit body/groove losses, and two-step sequence conditioning over previous windows.",
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Train a scratch retrieval-conditioned accompaniment generator with multi-window sequence conditioning and benchmark it.")
    ap.add_argument("--out-root", type=Path, default=REPO_ROOT / "Desktop Outputs" / "dggr_retrieval_body_style_sequence_runs")
