...
def _shift_pad_mel(x: torch.Tensor, shift: int) -> torch.Tensor:
    if shift == 0:
        return x
...
def _corrupt_prev_context(prev_mel: torch.Tensor, struct_mel: torch.Tensor) -> torch.Tensor:
    if float(prev_mel.abs().sum().item()) == 0.0:
        return prev_mel
    out = prev_mel.clone()
    b = int(out.shape[0])
    for i in range(b):
        shift = int(np.random.randint(-24, 25))
        cur = _shift_pad_mel(out[i : i + 1], shift)
        if np.random.rand() < 0.8:
            cur = smooth_mel_tensor(cur, time_kernel=9, freq_kernel=3)
        if np.random.rand() < 0.6:
            beta = float(np.random.uniform(0.20, 0.55))
            cur = (1.0 - beta) * cur + beta * struct_mel[i : i + 1]
        if np.random.rand() < 0.7:
            noise = 0.02 * torch.randn_like(cur)
            cur = cur + noise
        if np.random.rand() < 0.5:
            hf_start = int(np.random.randint(40, 60))
            cur[:, :, hf_start:, :] = 0.75 * cur[:, :, hf_start:, :] + 0.25 * struct_mel[i : i + 1, :, hf_start:, :]
        out[i : i + 1] = cur
...
def train(cfg: TrainConfig) -> Dict[str, Any]:
    ...
    if cfg.init_checkpoint and Path(cfg.init_checkpoint).exists():
        payload = torch.load(str(cfg.init_checkpoint), map_location=device, weights_only=False)
    ...
    for epoch in range(1, int(cfg.epochs) + 1):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            prev_in = _corrupt_prev_context(prev_mel, struct)
            prev2_in = _corrupt_prev_context(prev2_mel, struct)
            pred = model(struct, donor, prev_in, prev2_in, cond_feat, genre, context_count)
            ...
            long_tail = max(64, target.shape[-1] // 3)
            loss_tail = F.l1_loss(pred[:, :, :, -long_tail:], target[:, :, :, -long_tail:])
            loss_tail_dt = F.l1_loss(
                pred[:, :, :, -long_tail + 1 :] - pred[:, :, :, -long_tail:-1],
                target[:, :, :, -long_tail + 1 :] - target[:, :, :, -long_tail:-1],
            )
            loss = (
                0.82 * loss_l1 +
                0.28 * loss_dt +
    ...
    report_lines = [
        ...
        "- New family: guarded blockwise continuation fine-tune from the best 9-second checkpoints, with corrupted previous-context training and stronger final-region losses.",
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Fine-tune the 9-second block generator for guarded continuation robustness and benchmark it.")
    ap.add_argument("--out-root", type=Path, default=REPO_ROOT / "Desktop Outputs" / "dggr_retrieval_body_style_block_guarded_runs")
